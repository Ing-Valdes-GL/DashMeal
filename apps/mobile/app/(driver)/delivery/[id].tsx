...
const NEXT_ACTIONS: Partial<Record<Status, Array<{ status: Status; label: string; icon: string; primary: boolean }>>> = {
  assigned:   [{ status: "picked_up", label: "Commande récupérée", icon: "bag-check-outline", primary: true }],
  picked_up:  [{ status: "on_the_way", label: "En route vers le client", icon: "navigate-outline", primary: true }],
  on_the_way: [
    { status: "delivered",  label: "Livraison confirmée", icon: "checkmark-circle-outline", primary: true },
    { status: "failed",     label: "Échec de livraison",  icon: "close-circle-outline",     primary: false },
  ],
};
...
export default function DriverDeliveryDetailScreen() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: () => apiGet<{ data: DeliveryDetail }>(`/delivery/my-deliveries/${id}`),
    enabled: !!id,
  });

  const delivery = data?.data;
  const isActive = delivery && ["assigned", "picked_up", "on_the_way"].includes(delivery.status);
  ...
  const statusMutation = useMutation({
    mutationFn: (newStatus: Status) =>
      apiPatch(`/delivery/${id}/status`, { status: newStatus, note: statusNote || undefined }),
    onSuccess: () => {
  ...
  const statusColor = STATUS_COLOR[delivery.status];
  const actions = NEXT_ACTIONS[delivery.status] ?? [];

  // Map URL: branch as origin for "assigned", delivery address for others
  ...
  return (
    <View style={styles.root}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.card}>
          ...
          {delivery.orders?.notes && (
            <View style={[styles.row, { marginTop: 8 }]}>
              ...
              <Text style={styles.notes}>{delivery.orders.notes}</Text>
            </View>
          )}
        </View>
        ...
        {/* Order items */}
        <View style={styles.card}>
          <Text style={styles.cardTitle}>Commande #{delivery.orders?.id?.slice(-6).toUpperCase()}</Text>
        ...
        <View style={styles.actionsBar}>
          {actions.map((action) => (
            <TouchableOpacity
              ...
              style={[
                styles.actionBtn,
                action.primary ? styles.actionBtnPrimary : styles.actionBtnDanger,
                statusMutation.isPending && styles.actionBtnDisabled,
              ]}
...
const styles = StyleSheet.create({
  ...
  cardTitle: { fontSize: 13, fontWeight: "700", color: Colors.text3, marginBottom: 12, textTransform: "uppercase", letterSpacing: 0.5 },
  ...
  address: { flex: 1, fontSize: 14, color: Colors.text2, lineHeight: 20 },
  notes: { flex: 1, fontSize: 13, color: Colors.text2, fontStyle: "italic" },
